...
! menta: added MESUO
      USE DATAPOOL, ONLY : TAUW, FR, MESNL, MESIN, MESDS, MESBF, MESBR, MESUO
!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! menta
         IF (MESUO .GE. 1) THEN
!          IF (((ISELECT.EQ.6 .OR. ISELECT.EQ.10 .OR. ISELECT.EQ.30) .AND. ISHALLOW(IP) .NE. 1) .AND. .NOT. LRECALC) THEN
           IF ((ISELECT.EQ.6 .OR. ISELECT.EQ.10 .OR. ISELECT.EQ.30) .AND. .NOT. LRECALC) THEN
             IF (IOBP(IP) == 0 .OR. IOBP(IP) == 4 .OR. IOBP(IP) == 3) THEN
              CALL UOST_SRCTRM(IP,ACLOC,CG(:,IP),WIND10,WINDTH,IMATRA,IMATDA)
             END IF
           END IF
         ENDIF
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
